...
public class UnrentCommand extends CommandAreaShop {
	...
	public String getHelp(CommandSender target) {
		if (target.hasPermission("areashop.unrent")) return plugin.getLanguageManager().getLang("help-unrent");
		else if (target.hasPermission("areashop.unrentown")) return plugin.getLanguageManager().getLang("help-unrentOwn");
		return null;
	}
	...
	public void execute(CommandSender sender, Command command, String[] args) {
		...
		if (sender.hasPermission("areashop.unrent")) {
			...
			if (sender.hasPermission("areashop.unrentown") && sender instanceof Player) {
				if (rent.getRenter().equals(((Player) sender).getName())) {
