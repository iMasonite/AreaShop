...
public class SellCommand extends CommandAreaShop {
	...
	public String getHelp(CommandSender target) {
		if (target.hasPermission("areashop.sell")) return plugin.getLanguageManager().getLang("help-sell");
		else if (target.hasPermission("areashop.sellown")) return plugin.getLanguageManager().getLang("help-sellOwn");
		return null;
	}
	...
	public void execute(CommandSender sender, Command command, String[] args) {
		...
		if (sender.hasPermission("areashop.sell")) {
			...
			if (sender.hasPermission("areashop.sellown") && sender instanceof Player) {
				if (buy.getBuyer().equals(((Player) sender).getName())) {
