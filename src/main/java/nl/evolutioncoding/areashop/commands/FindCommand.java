...
public class FindCommand extends CommandAreaShop {
	...
	public String getHelp(CommandSender target) {
		if (target.hasPermission("areashop.find")) return plugin.getLanguageManager().getLang("help-find");
		return null;
	}
	...
	public void execute(CommandSender sender, Command command, String[] args) {
		...
		if (args[1].equalsIgnoreCase("buy")) {
			...
			for (BuyRegion region : regions) {
				if (!region.isSold() && ((region.getPrice() <= balance && !maxPriceSet) || (region.getPrice() <= maxPrice && maxPriceSet)) && (group == null || group.isMember(region))) {
					results.add(region);
				}
			}
			if (!results.isEmpty()) {
			...
			for (RentRegion region : regions) {
				if (!region.isRented() && ((region.getPrice() <= balance && !maxPriceSet) || (region.getPrice() <= maxPrice && maxPriceSet)) && (group == null || group.isMember(region))) {
					results.add(region);
				}
			}
			if (!results.isEmpty()) {
