...
public class BuyRegion extends GeneralRegion {
	...
	public RegionState getState() {
		if (isSold() && isInResellingMode()) return RegionState.RESELL;
		else if (isSold() && !isInResellingMode()) return RegionState.SOLD;
		else return RegionState.FORSALE;
	}
	...
	public boolean isBuyer(Player player) {
		...
		else return isBuyer(player.getName());
	}
	
	public boolean isBuyer(String player) {
		String buyer = getBuyer();
		if (buyer == null || player == null) return false;
		else return getBuyer().equals(player);
	}
	
	/** Set the buyer of this region
	 * 
	 * @param buyer The UUID of the player that should be set as buyer */
	public void setBuyer(String playerName) {
		if (playerName == null) {
			setSetting("buy.buyer", null);
			setSetting("buy.buyerName", null);
		}
		else {
			setSetting("buy.buyer", playerName.toString());
			setSetting("buy.buyerName", playerName);
		}
	}
	
	/** Get the name of the player that owns this region
	 * 
	 * @return The name of the player that owns this region, if unavailable by UUID it will return the
	 *         old cached name, if that is unavailable it will return <UNKNOWN> */
	public String getPlayerName() {
		String result = getBuyer();
	...
	public boolean buy(Player player) {
		...
		if (player.hasPermission("areashop.buy")) {
			...
			if (!isSold() || (isInResellingMode() && !isBuyer(player))) {
				...
				if ((!isResell && plugin.getEconomy().has(player.getName(), getWorldName(), getPrice())) || (isResell && plugin.getEconomy().has(player.getName(), getWorldName(), getResellPrice()))) {
					String oldOwner = getBuyer();
					if (isResell && oldOwner != null) {
						clearFriends();
						double resellPrice = getResellPrice();
						/* Transfer the money to the previous owner */
						EconomyResponse r = plugin.getEconomy().withdrawPlayer(player.getName(), getWorldName(), getResellPrice());
						...
						if (oldOwnerPlayer != null) {
							r = plugin.getEconomy().depositPlayer(oldOwner, getWorldName(), getResellPrice());
							if (!r.transactionSuccess()) {
					...
					plugin.message(player, "buy-lowMoney", plugin.formatCurrency(plugin.getEconomy().getBalance(player.getName(), getWorldName())), requiredMoney);
				}
			}
			else {
				if (isBuyer(player)) {
	...
	public void sell(boolean giveMoneyBack) {
		...
		if (moneyBack > 0 && giveMoneyBack) {
			...
			if (player != null) {
				...
				boolean error = false;
				try {
					response = plugin.getEconomy().depositPlayer(getBuyer(), getWorldName(), moneyBack);
				}
				catch (Exception e) {
					error = true;
				}
	...
	public boolean checkInactive() {
		if (isDeleted() || !isSold()) return false;
		OfflinePlayer player = Bukkit.getOfflinePlayer(getBuyer());
