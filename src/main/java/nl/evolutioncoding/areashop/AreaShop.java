...
public final class AreaShop extends JavaPlugin implements AreaShopInterface {
	...
	public boolean checkTimeFormat(String time) {
		...
		if (time == null || time.length() <= 1 || time.indexOf(' ') == -1 || time.indexOf(' ') >= (time.length() - 1)) return false;
		...
		if (!identifiers.contains(suffix)) return false;
	 ...
	 * @return The duration in milliseconds translated from the durationstring, or if it is invalid
	 *         then 0 */
	public long durationStringToLong(String duration) {
		if (duration == null) return 0;
		else if (duration.equalsIgnoreCase("disabled") || duration.equalsIgnoreCase("unlimited")) return -1;
	...
	/** Get setting from config that could be only a number indicating seconds or a string indicating a
	 * duration string
	 * 
	...
	public long getDurationFromSecondsOrString(String path) {
		if (getConfig().isLong(path)) return getConfig().getLong(path) * 1000;
		else return durationStringToLong(getConfig().getString(path));
	}
	
	/** Get setting from config that could be only a number indicating minutes or a string indicating a
	 * duration string
	 * 
	 * @param path Path of the setting to read
	 * @return milliseconds that the setting indicates */
	public long getDurationFromMinutesOrString(String path) {
		if (getConfig().isLong(path)) return getConfig().getLong(path) * 60 * 1000;
		else return durationStringToLong(getConfig().getString(path));
	}
	...
	/** Reload all files of the plugin and update all regions confirmationReceiver The CommandSender
	 * that should receive confirmation messages, null for nobody */
